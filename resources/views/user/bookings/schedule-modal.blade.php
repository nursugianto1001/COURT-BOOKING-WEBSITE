<div x-data="scheduleModal({{ $booking->id }}, {{ $booking->court_id }}, {{ $booking->duration }})"
     x-show="isOpen"
     x-cloak
     @open-schedule-modal.window="openModal($event.detail.bookingId)"
     @keydown.escape.window="closeModal"
     class="fixed inset-0 z-50 overflow-y-auto">
    <!-- Backdrop dengan blur effect -->
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        <div class="fixed inset-0 transition-opacity backdrop-blur-sm" aria-hidden="true"
             @click="closeModal">
            <div class="absolute inset-0 bg-gray-900/70"></div>
        </div>

        <!-- Modal Content dengan efek mengambang -->
        <div class="relative inline-block w-full max-w-4xl overflow-hidden text-left align-middle transition-all transform bg-white shadow-[0_10px_40px_-5px_rgba(0,0,0,0.3)] rounded-2xl">
            <!-- Header dengan gradient -->
            <div class="bg-gradient-to-r from-primary/10 to-transparent p-6">
                <div class="flex justify-between items-center">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-800" x-text="selectedCourt?.name || ''"></h3>
                        <p class="text-gray-600 mt-1 flex items-center gap-2">
                            <svg class="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                      d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                      d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                            </svg>
                            <span x-text="selectedCourt?.location || ''"></span>
                        </p>
                    </div>
                    <button @click="closeModal" 
                            type="button"
                            class="p-2 hover:bg-gray-100 rounded-full transition-colors">
                        <svg class="h-6 w-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                </div>
            </div>

            <div class="p-6">
                <!-- Calendar Navigation -->
                <div class="flex items-center justify-between mb-6">
                    <button type="button" 
                            @click="previousWeek()"
                            class="p-2 text-gray-600 hover:text-primary transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                        </svg>
                    </button>

                    <div class="flex space-x-3 overflow-x-auto pb-2 scrollbar-hide flex-grow mx-4">
                        <template x-for="date in dates" :key="date.value">
                            <button type="button"
                                    @click="selectDate(date.value)"
                                    :class="{
                                        'bg-primary shadow-lg shadow-primary/30 transform scale-105 text-white': selectedDate === date.value,
                                        'bg-white hover:bg-gray-50 hover:shadow-md text-gray-800': selectedDate !== date.value
                                    }"
                                    class="flex flex-col items-center px-4 py-3 rounded-xl border transition-all duration-300 min-w-[90px]">
                                <span class="text-xs" x-text="date.day"></span>
                                <span class="text-lg font-bold mt-1" x-text="date.date"></span>
                                <span class="text-xs font-medium" x-text="date.month"></span>
                            </button>
                        </template>
                    </div>

                    <button type="button" 
                            @click="nextWeek()"
                            class="p-2 text-gray-600 hover:text-primary transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                        </svg>
                    </button>
                </div>

                <!-- Time slots dengan efek mengambang default -->
                <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3 mt-4">
                    <template x-for="slot in availableTimeSlots" :key="slot.time">
                        <button type="button"
                                @click="selectTimeSlot(slot)"
                                :disabled="slot.status === 'booked' || slot.status === 'holiday'"
                                :class="getTimeSlotClasses(slot)"
                                class="p-4 rounded-xl text-sm font-medium transition-all duration-300 relative overflow-hidden">
                            <div class="text-base font-bold" x-text="formatTime(slot.time)"></div>
                            <div class="text-xs mt-1">1 Jam</div>
                            <div class="text-xs mt-1 font-medium" 
                                 :class="{
                                     'text-red-500': slot.status === 'booked',
                                     'text-yellow-500': slot.status === 'holiday',
                                     'text-green-500': slot.status === 'available'
                                 }"
                                 x-text="getStatusText(slot.status)">
                            </div>
                            <!-- Holiday overlay dengan efek glass morphism -->
                            <div x-show="slot.status === 'holiday'"
                                 class="absolute inset-0 backdrop-blur-[2px] bg-gradient-to-b from-gray-50/80 to-gray-100/80
                                        flex items-center justify-center">
                                <span class="text-xs font-medium px-3 py-1.5 rounded-full
                                           bg-gradient-to-r from-yellow-50 to-orange-50 
                                           border border-yellow-200/50 shadow-sm">
                                    Libur
                                </span>
                            </div>
                        </button>
                    </template>
                </div>

                <!-- Total Price & Submit dengan gradient dan efek hover -->
                <div class="flex items-center justify-between mt-8 pt-6 border-t">
                    <div>
                        <p class="text-sm text-gray-600">Total Pembayaran</p>
                        <p class="text-3xl font-bold bg-gradient-to-r from-primary to-primary/80 bg-clip-text text-transparent" 
                           x-text="formatPrice(totalPrice)">
                        </p>
                    </div>
                    <button type="button"
                            @click="submitSchedule"
                            :disabled="!isFormValid"
                            :class="{'opacity-50 cursor-not-allowed': !isFormValid}"
                            class="px-8 py-3 bg-gradient-to-r from-primary to-primary/90 text-white rounded-xl 
                                   hover:shadow-lg hover:shadow-primary/30 transform hover:scale-105 
                                   transition-all duration-300">
                        Konfirmasi Booking
                    </button>
                </div>
            </div>
        </div>
    </div>
</div>

<!-- Tambahkan style untuk scrollbar -->
<style>
    .scrollbar-hide::-webkit-scrollbar {
        display: none;
    }
    .scrollbar-hide {
        -ms-overflow-style: none;
        scrollbar-width: none;
    }
</style>

@push('scripts')
<script>
document.addEventListener('alpine:init', () => {
    Alpine.data('scheduleModal', (bookingId, courtId, duration) => ({
        isOpen: false,
        bookingId: bookingId,
        courtId: courtId,
        duration: duration,
        selectedCourt: null,
        availableTimeSlots: [],
        selectedSlots: [],
        dates: [],
        selectedDate: null,
        errorMessage: null,
        currentWeekOffset: 0,

        init() {
            this.selectedDate = new Date().toISOString().split('T')[0];
            this.fetchCourtDetails();
            this.generateDates();
        },

        async fetchCourtDetails() {
            try {
                console.log('Fetching court details for ID:', this.courtId);
                const response = await fetch(`/api/courts/${this.courtId}`);
                const data = await response.json();
                
                console.log('Court details received:', data);
                
                if (!response.ok) {
                    throw new Error('Failed to fetch court details');
                }
                
                this.selectedCourt = data;
                console.log('Selected court set:', this.selectedCourt);
            } catch (error) {
                console.error('Error fetching court details:', error);
                this.errorMessage = 'Gagal memuat detail lapangan';
            }
        },

        async openModal(bookingId) {
            console.log('Opening modal with bookingId:', bookingId);
            this.isOpen = true;
            await this.fetchSchedules();
        },

        async fetchSchedules() {
            try {
                console.log('Fetching schedules for date:', this.selectedDate);
                const response = await fetch(`/user/bookings/${this.bookingId}/schedules?date=${this.selectedDate}`);
                const data = await response.json();
                console.log('Schedule data received:', data);

                if (data.schedules) {
                    this.availableTimeSlots = data.schedules.map(slot => ({
                        ...slot,
                        formattedTime: this.formatTime(slot.time)
                    }));
                }
                
                console.log('Current dates:', this.dates);
                console.log('Selected date:', this.selectedDate);
                
                this.errorMessage = null;
            } catch (error) {
                console.error('Error fetching schedules:', error);
                this.errorMessage = 'Gagal memuat jadwal';
            }
        },

        async generateDates() {
            const dates = [];
            const today = new Date();
            
            // Simpan tanggal yang dipilih sebelumnya
            const previousSelectedDate = this.selectedDate;
            
            // Tambah offset minggu ke tanggal awal
            const startDate = new Date(today);
            startDate.setDate(today.getDate() + (this.currentWeekOffset * 7));
            
            for (let i = 0; i < 7; i++) {
                const date = new Date(startDate);
                date.setDate(startDate.getDate() + i);
                
                dates.push({
                    value: date.toISOString().split('T')[0],
                    day: date.toLocaleDateString('id-ID', { weekday: 'short' }),
                    date: date.getDate(),
                    month: date.toLocaleDateString('id-ID', { month: 'short' })
                });
            }
            this.dates = dates;

            // Jika ada tanggal yang dipilih sebelumnya dalam range baru, gunakan itu
            const dateInRange = this.dates.find(d => d.value === previousSelectedDate);
            const dateToSelect = dateInRange ? previousSelectedDate : dates[0].value;
            
            // Fetch jadwal untuk tanggal yang dipilih
            if (this.selectedDate !== dateToSelect) {
                await this.selectDate(dateToSelect);
            } else {
                await this.fetchSchedules();
            }
        },

        async selectDate(date) {
            if (!date) return;
            
            console.log('Selecting date:', date);
            this.selectedDate = date;
            this.selectedSlots = []; // Reset selected slots when changing date
            await this.fetchSchedules();
        },

        selectTimeSlot(slot) {
            if (!this.canSelectSlot(slot)) return;

            const index = this.selectedSlots.findIndex(s => s.time === slot.time);
            
            if (index > -1) {
                // Jika slot sudah dipilih, hapus slot ini dan semua slot setelahnya
                this.selectedSlots = this.selectedSlots.slice(0, index);
            } else {
                // Jika slot belum dipilih
                if (this.selectedSlots.length === 0) {
                    // Jika belum ada slot yang dipilih, tambahkan slot ini
                    this.selectedSlots = [slot];
                } else {
                    // Cek apakah slot ini berurutan dengan slot terakhir yang dipilih
                    const lastSlot = this.selectedSlots[this.selectedSlots.length - 1];
                    const lastTime = new Date(`2000-01-01 ${lastSlot.time}`);
                    const currentTime = new Date(`2000-01-01 ${slot.time}`);
                    const diffHours = (currentTime - lastTime) / (1000 * 60 * 60);

                    if (diffHours === 1) {
                        // Jika slot berurutan, tambahkan ke selection
                        this.selectedSlots.push(slot);
                    } else {
                        // Jika tidak berurutan, mulai selection baru
                        this.selectedSlots = [slot];
                    }
                }
            }
        },

        isSelected(slot) {
            return this.selectedSlots.some(s => s.time === slot.time);
        },

        canSelectSlot(slot) {
            if (slot.status !== 'available') return false;
            return true;
        },

        get canSubmit() {
            return this.selectedSlots.length === this.duration;
        },

        get totalPrice() {
            if (!this.selectedCourt || !this.selectedSlots.length) return 0;
            return this.selectedCourt.price_per_hour * this.selectedSlots.length;
        },

        formatPrice(price) {
            return new Intl.NumberFormat('id-ID', {
                style: 'currency',
                currency: 'IDR',
                minimumFractionDigits: 0
            }).format(price);
        },

        async submitSchedule() {
            try {
                const response = await fetch(`/user/bookings/${this.bookingId}/schedule`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    },
                    body: JSON.stringify({
                        slots: this.selectedSlots,
                        duration: this.selectedSlots.length,
                        date: this.selectedDate
                    })
                });

                const data = await response.json();

                if (response.ok) {
                    window.location.reload();
                } else {
                    throw new Error(data.message || 'Gagal memperbarui jadwal');
                }
            } catch (error) {
                console.error('Error updating schedule:', error);
                this.errorMessage = error.message || 'Terjadi kesalahan saat memperbarui jadwal';
            }
        },

        formatTime(time) {
            if (!time) return '';
            const [hours, minutes] = time.split(':');
            return `${hours}:${minutes}`;
        },

        getTimeSlotClasses(slot) {
            const baseClasses = 'p-4 rounded-xl text-sm font-medium transition-all duration-300 relative overflow-hidden ' +
                              'shadow-[0_2px_10px_-3px_rgba(0,0,0,0.07)] hover:shadow-[0_8px_20px_-4px_rgba(0,0,0,0.1)] ' +
                              'border border-gray-100 hover:border-gray-200 transform hover:scale-105 hover:-translate-y-1';

            if (slot.status === 'booked' || slot.status === 'holiday') {
                return `${baseClasses} bg-white text-gray-400`;
            }
            
            if (this.isSelected(slot)) {
                return `${baseClasses} bg-primary text-white shadow-lg shadow-primary/30 transform scale-105`;
            }
            
            if (!this.canSelectSlot(slot)) {
                return `${baseClasses} bg-white text-gray-400 cursor-not-allowed opacity-50`;
            }
            
            return `${baseClasses} bg-white text-gray-800`;
        },

        getStatusText(status) {
            switch(status) {
                case 'holiday':
                    return 'Libur';
                case 'booked':
                    return 'Sudah Dibooking';
                case 'available':
                    return 'Tersedia';
                default:
                    return '';
            }
        },

        findBooking() {
            return this.bookings.find(b => b.id === this.bookingId);
        },

        closeModal() {
            this.isOpen = false;
            this.resetForm();
        },

        resetForm() {
            this.availableTimeSlots = [];
            this.selectedSlots = [];
            this.selectedDate = null;
            this.errorMessage = null;
        },

        nextWeek() {
            this.currentWeekOffset++;
            this.generateDates();
        },

        previousWeek() {
            if (this.currentWeekOffset > 0) {
                this.currentWeekOffset--;
                this.generateDates();
            }
        }
    }));
});
</script>
@endpush
